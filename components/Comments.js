...
export default function Comments({ videoId }) {
  ...
  const post = async (event) => {
    ...
    try {
      const res = await fetch("/api/comments", {
        ...
        body: JSON.stringify({ videoId, text: draft }),
      });
      const data = await res.json().catch(() => null);
  ...
  const remove = async (comment) => {
    ...
    try {
      const res = await fetch(
        `/api/comments?videoId=${encodeURIComponent(videoId)}&id=${encodeURIComponent(comment.id)}`,
        { method: "DELETE" }
      );
      const data = await res.json().catch(() => null);
      if (!res.ok) {
  ...
  return (
    <section className="comments card" aria-labelledby="comments-title">
      <h2 id="comments-title" className="notes-title">
        Comments{comments && comments.length ? ` (${comments.length})` : ""}
      </h2>

      {comments === null ? <p className="muted small">Loading comments…</p> : null}
            ...
            <li key={comment.id} className="comment">
              <div className="comment-head">
                ...
                {comment.email && !comment.mine ? (
                  <span className="muted small comment-email">{comment.email}</span>
                ) : null}
                <span className="muted small">{when(comment.at)}</span>
                {comment.canDelete ? (
                  <button
                    type="button"
