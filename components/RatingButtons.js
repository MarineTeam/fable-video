...
export default function RatingButtons({ videoId, initialVote = null }) {
    ...
    try {
      const res = target
        ? await fetch("/api/rating", {
            ...
            body: JSON.stringify({ videoId, vote: target }),
          })
        : await fetch(`/api/rating?videoId=${encodeURIComponent(videoId)}`, {
            method: "DELETE",
          });
