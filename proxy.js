...
export const config = {
  matcher: [
    ...
    // assets (manifest, service worker, icons — including the admin-set
    // icon route, api/app-icon) are excluded so they are served cleanly
    // without session-cookie churn. Scheduled jobs (api/cron) are excluded
    // too: their caller is Vercel's cron runner, which has no session and
    // whose requests may carry no country, so geo enforcement would refuse
    // them. CRON_SECRET is their gate (lib/cronAuth.js).
    "/((?!_next/static|_next/image|favicon.ico|sitemap.xml|robots.txt|manifest.webmanifest|sw.js|icon-192.png|icon-512.png|icon-maskable-512.png|apple-touch-icon.png|api/app-icon|api/cron/).*)",
  ],
};
