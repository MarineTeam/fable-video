...
export function videoMatchesQuery(video, query) {
  ...
  return passage ? passageMatches(cachedReferences(video), passage) : false;
}

// The homepage re-runs the predicate on every debounced keystroke over the
// whole loaded list; parsing each video's notes each time is wasted work. A
...
function cachedReferences(video) {
  if (!video || typeof video !== "object") return [];
  const text = `${video.title || ""}\n${video.notes || ""}`;
  const hit = referenceCache.get(video);
  if (hit && hit.text === text) return hit.refs;
  const refs = videoReferences(video);
  referenceCache.set(video, { text, refs });
  return refs;
}
