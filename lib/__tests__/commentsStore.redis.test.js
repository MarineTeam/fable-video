...
describe.skipIf(shouldSkip)("comments on a real redis-server", () => {
  ...
  it("refuses a comment once the video is at its cap, and writes nothing", async () => {
    ...
    await r.call("HSET", "fablevideo:comments:vid-1", ...fill);
    expect(await store.addComment("vid-1", jane)).toEqual({ ok: false, error: "full" });
    expect(await r.call("HLEN", "fablevideo:comments:vid-1")).toBe(MAX_COMMENTS_PER_VIDEO);
  });
  ...
  it("deletes one comment, and ignores an id that is not one", async () => {
    ...
    await r.call("HSET", "fablevideo:comments:vid-1", "not-an-id", "x");
    await store.deleteComment("vid-1", "not-an-id");
    expect(await r.call("HEXISTS", "fablevideo:comments:vid-1", "not-an-id")).toBe(1);
    await r.call("HDEL", "fablevideo:comments:vid-1", "not-an-id");
