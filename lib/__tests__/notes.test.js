...
describe("videoMatchesQuery: word stems", () => {
  const video = { title: "Baptized in the Jordan", notes: "Forgiveness and grace" };

  it("finds another form of the same word in title or notes", () => {
    expect(videoMatchesQuery(video, "baptism")).toBe(true);
    expect(videoMatchesQuery(video, "forgiving")).toBe(true);
  });

  it("finds a multi-word query across title and notes, in any order", () => {
    expect(videoMatchesQuery(video, "grace baptise")).toBe(true);
    expect(videoMatchesQuery(video, "grace wilderness")).toBe(false);
  });

  it("only ADDS matches — plain substrings still match as before", () => {
    expect(videoMatchesQuery(video, "jord")).toBe(true);
    expect(videoMatchesQuery(video, "ptiz")).toBe(true);
  });

  it("does not let stems widen a PASSAGE query to the whole book", () => {
    const phil4 = { title: "Contentment", notes: "Philippians 4:10-20" };
    expect(videoMatchesQuery(phil4, "Philippians 2")).toBe(false);
    expect(videoMatchesQuery(phil4, "Philippians")).toBe(true);
    // A stray "2" elsewhere must not rescue it: the words are all there, but
    // the passage is not, and the passage is what was asked for.
    const stray = { title: "Contentment, part 2", notes: "Philippians 4:10-20" };
    expect(videoMatchesQuery(stray, "Philippians 2")).toBe(false);
  });
});
