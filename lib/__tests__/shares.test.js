...
describe("groupSharesByEmail", () => {
  it("returns one row per recipient, sorted by email", () => {
    const shares = [
      { id: "s2", email: "bob@x.com", createdAt: "2026-01-01T00:00:00Z", expiresAt: "2026-02-01T00:00:00Z", emailedAt: null, viewCount: 1 },
      { id: "s1", email: "alice@x.com", createdAt: "2026-01-01T00:00:00Z", expiresAt: "2026-02-01T00:00:00Z", emailedAt: null, viewCount: 0 },
    ];
    const grouped = groupSharesByEmail(shares);
    expect(grouped.map((g) => g.email)).toEqual(["alice@x.com", "bob@x.com"]);
  });

  it("merges more than one live share for the same recipient into one row", () => {
    const shares = [
      { id: "old", email: "a@x.com", createdAt: "2026-01-01T00:00:00Z", expiresAt: "2026-01-15T00:00:00Z", emailedAt: "2026-01-01T00:00:00Z", viewCount: 2 },
      { id: "new", email: "a@x.com", createdAt: "2026-01-10T00:00:00Z", expiresAt: "2026-02-01T00:00:00Z", emailedAt: null, viewCount: 1 },
    ];
    const grouped = groupSharesByEmail(shares);
    expect(grouped).toHaveLength(1);
    expect(grouped[0].ids.sort()).toEqual(["new", "old"]);
    expect(grouped[0].createdAt).toBe("2026-01-01T00:00:00Z");
    expect(grouped[0].expiresAt).toBe("2026-02-01T00:00:00Z");
    expect(grouped[0].emailedAt).toBe("2026-01-01T00:00:00Z");
    expect(grouped[0].viewCount).toBe(3);
  });

  it("returns an empty array for no shares", () => {
    expect(groupSharesByEmail([])).toEqual([]);
  });
});
