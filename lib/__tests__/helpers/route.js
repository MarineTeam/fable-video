...
export function mockRes() {
  const res = {
    ...
    // Records the payload so a route that writes a non-JSON body (the
    // podcast feed's XML, for example) can be asserted on, not just its
    // status code. json() still sets res.body the same way it always did.
    end(payload) {
