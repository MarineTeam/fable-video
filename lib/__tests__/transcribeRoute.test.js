...
const markTranscribePending = vi.fn(async () => {});
const clearTranscribePending = vi.fn(async () => {});
...
vi.mock("../guard", () => ({
  requireCapability: async (req, res) => {
    ...
    // The real guard returns the whole access record, not the email. This
    // stub used to return the bare email, which hid the route logging and
    // rate-limiting the record itself.
    return { email: admin, staffScope: null, videoScope: null };
  },
}));
const limitedAs = [];
vi.mock("../ratelimit", () => ({
  allowRequest: async (name, id) => {
    limitedAs.push(id);
    return allowed;
  },
}));
...
vi.mock("../captionsStore", () => ({
  setTranscript: (...args) => setTranscript(...args),
  markTranscribePending: (...args) => markTranscribePending(...args),
  clearTranscribePending: (...args) => clearTranscribePending(...args),
}));
...
beforeEach(() => {
  ...
  markTranscribePending.mockClear();
  clearTranscribePending.mockClear();
  logAction.mockClear();
});
...
describe("the paid branch", () => {
  ...
  it("asks for chapters when the admin ticked the box", async () => {
    ...
    // Logged and rate-limited as the caller's email, never the access record.
    expect(logAction.mock.calls[0][0]).toBe("admin@example.com");
    expect(limitedAs.at(-1)).toBe("admin@example.com");
  });
});

describe("the queue marker", () => {
  it("is recorded when a paid job is queued", async () => {
    // Without it the admin video list has nothing to collect, and the second
    // click is mandatory again — the exact failure this marker removes.
    await call({ guid: "vid-1" });
    expect(markTranscribePending).toHaveBeenCalledWith("vid-1");
  });

  it("is NOT recorded when the job was refused", async () => {
    // A marker for a job that never started would spend two bunny calls per
    // admin page load for a day, discovering nothing.
    allowed = false;
    await call({ guid: "vid-1" });
    expect(markTranscribePending).not.toHaveBeenCalled();
  });

  it("is cleared by a successful manual ingest", async () => {
    video = { guid: "vid-1", captions: [{ srclang: "en" }] };
    await call({ guid: "vid-1", ingest: true });
    expect(clearTranscribePending).toHaveBeenCalledWith("vid-1");
  });

  it("survives a marker write that fails", async () => {
    // Best-effort by contract: losing the marker costs a click, and must
    // never fail the request that already spent the money.
    markTranscribePending.mockImplementationOnce(async () => {
