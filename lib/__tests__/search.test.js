...
describe("searchLibrary", () => {
  ...
  it("finds a video by a TRANSLATION the store matched, and only videos in the list", () => {
    const result = searchLibrary({
      videos: library,
      translatedIds: new Set(["vid-3", "vid-not-in-library"]),
      query: "oración",
    });
    expect(result.videos.map((v) => v.id)).toEqual(["vid-3"]);
  });

  it("ignores a translated match for a query too short to search transcripts with", () => {
    const result = searchLibrary({ videos: library, translatedIds: new Set(["vid-2"]), query: "zq" });
...
describe("linkedQuery / passageSearchHref", () => {
  ...
  it("builds a link that reads back to the same passage", () => {
    ...
    expect(href).toBe("/?q=Philippians%201%3A27%E2%80%932%3A11");
    const q = new URL(href, "https://x.test").searchParams.get("q");
    expect(linkedQuery(q)).toBe("Philippians 1:27–2:11");
  });
});

describe("searchLibrary: passages reach past the homepage cap too", () => {
  it("finds a passage in the server half, through the same matcher", () => {
    const videos = [
      { id: "a", title: "Humility", notes: "Phil 1:27-2:11" },
      { id: "b", title: "Contentment", notes: "Philippians 4:10-20" },
      { id: "c", title: "Harbour tour", notes: "" },
    ];
    const result = searchLibrary({ videos, transcripts: {}, query: "Philippians 2", limit: 10 });
    expect(result.videos.map((v) => v.id)).toEqual(["a"]);
  });
});
