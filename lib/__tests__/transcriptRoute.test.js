...
describe("publish window", () => {
  ...
  // Per-group windows reach this enforcement point too: a member of an
  // early-access group reads the transcript before the public release, and a
  // non-member still does not.
  it("follows a per-group window for a member, and only for a member", async () => {
    const future = new Date(Date.now() + 86_400_000).toISOString();
    const past = new Date(Date.now() - 86_400_000).toISOString();
    schedule = { publishAt: future, expiresAt: null, groups: { youth: { publishAt: past, expiresAt: null } } };
    access = { ...approved, groupIds: ["youth"] };
    expect((await call()).statusCode).toBe(200);
    access = { ...approved, groupIds: ["crew"] };
