...
function redisState({
  ...
  hget.mockImplementation(async (key, field) => {
    if (key === "fablevideo:app_icon") return appIcon[field] ?? null;
...
describe("GET /api/manifest", () => {
  ...
  it("offers the built-in icons, maskable included, when no icon is set", async () => {
    redisState();
    const res = await callRoute(manifestRoute);
  ...
  it("offers the admin-set icon through its versioned URL, and NOT as maskable", async () => {
    // An arbitrary upload has no safe zone; offered as maskable, Android would
    // clip its edges.
    redisState({ appIcon: { version: "vabc123def456" } });
    const res = await callRoute(manifestRoute);
  ...
  it("ignores a stored version that is not the expected shape", async () => {
    redisState({ appIcon: { version: 123456789012 } });
    const res = await callRoute(manifestRoute);
    expect(res.body.icons[0].src).toBe("/icon-192.png");
  });
