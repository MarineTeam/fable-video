...
// The escalation the subset rule cannot see: lib/roles.js resolveAccess()
// derives viewer approval from holding any capability, so a role assignment is
// also a grant of library access — which viewers.manage, not roles.manage, is
// supposed to gate.
describe("assignmentNeedsViewerManage", () => {
  const ROLES_ONLY = [CAP.ROLES_MANAGE];
  const ROLES_AND_VIEWERS = [CAP.ROLES_MANAGE, CAP.VIEWERS_MANAGE];

  it("refuses a roles.manage-only actor giving a stranger their first role", () => {
    expect(
      assignmentNeedsViewerManage({
        owner: false,
        actorCaps: ROLES_ONLY,
        grantedCaps: [CAP.AUDIT_READ],
        targetApproved: false,
      })
    ).toBe(true);
  });

  it("allows it once the actor also holds viewers.manage", () => {
    expect(
      assignmentNeedsViewerManage({
        owner: false,
        actorCaps: ROLES_AND_VIEWERS,
        grantedCaps: [CAP.AUDIT_READ],
        targetApproved: false,
      })
    ).toBe(false);
  ...
  it("allows it when the target can already see the library", () => {
    expect(
      assignmentNeedsViewerManage({
        owner: false,
        actorCaps: ROLES_ONLY,
  ...
  it("never blocks removing every role, which grants nothing", () => {
    expect(
      assignmentNeedsViewerManage({
        owner: false,
        actorCaps: ROLES_ONLY,
  ...
  it("ignores unknown capability strings, so a junk role is not a foothold", () => {
    expect(
      assignmentNeedsViewerManage({
        owner: false,
        actorCaps: ROLES_ONLY,
        grantedCaps: ["not.a.capability"],
        targetApproved: false,
      })
    ).toBe(false);
  });
