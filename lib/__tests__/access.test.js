...
const { capabilityHolders, resolveAccess } = await import("../roles");
...
// Who a capability-addressed notification reaches. Derived from the role
// table rather than hardcoded, so moving a capability between roles moves the
// recipients with it instead of quietly sending to the wrong people.
describe("capabilityHolders", () => {
  it("returns the admins plus the ADMIN_EMAILS seed for people-management", async () => {
    process.env.ADMIN_EMAILS = "root@example.com";
    hgetall.mockImplementation(async (key) =>
      key === "fablevideo:roles"
        ? {
            "boss@example.com": "admin",
            "mate@example.com": "manager",
            "crew@example.com": "viewer",
          }
        : {}
    );
    const holders = await capabilityHolders(CAP.PEOPLE);
    expect(holders.sort()).toEqual(["boss@example.com", "root@example.com"]);
  });

  // A manager holds SHARES but not PEOPLE — the two lists must differ, or the
  // capability is not actually doing the selecting.
  it("returns a wider list for a capability managers also hold", async () => {
    process.env.ADMIN_EMAILS = "root@example.com";
    hgetall.mockImplementation(async (key) =>
      key === "fablevideo:roles"
        ? { "mate@example.com": "manager", "crew@example.com": "viewer" }
        : {}
    );
    const holders = await capabilityHolders(CAP.SHARES);
    expect(holders.sort()).toEqual(["mate@example.com", "root@example.com"]);
  });

  it("returns the seed alone when no role is stored", async () => {
    process.env.ADMIN_EMAILS = "root@example.com";
    hgetall.mockImplementation(async () => ({}));
    expect(await capabilityHolders(CAP.PEOPLE)).toEqual(["root@example.com"]);
  });
});
