...
describe("plain viewers", () => {
  ...
  // What per-group publish windows are keyed on. Every tag, normalized the
  // way group ids are, whether or not the group restricts anything.
  it("carries the viewer's group ids, for per-group publish windows", async () => {
    stub({ viewerMeta: { addedAt: "x", tags: ["Team A", "Youth", "team a"] } });
    expect((await resolveAccess("crew@example.com")).groupIds).toEqual(["team a", "youth"]);
  });
