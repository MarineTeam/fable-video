// lib/store.js ratings functions end to end against a REAL redis-server: the store's own
...
describe.skipIf(shouldSkip)("ratings store on a real redis-server", () => {
  ...
  it("puts the vote in the viewer’s hash and the count in the counters hash", async () => {
    expect(await store.recordRating("a@example.com", "vid-1", "up")).toBe(true);
    expect(await r.call("HGETALL", "fablevideo:ratings:a@example.com")).toEqual(["vid-1", "up"]);
    expect(await r.call("HGETALL", "fablevideo:rating_counts")).toEqual(["vid-1:up", "1"]);
  });
  ...
  it("reports a repeat as unchanged", async () => {
    await store.recordRating("a@example.com", "vid-1", "up");
    expect(await store.recordRating("a@example.com", "vid-1", "up")).toBe(false);
  });
  ...
  it("recounts from every viewer’s hash and replaces drifted totals", async () => {
    ...
    await r.call("HSET", "fablevideo:rating_counts", "vid-1:up", "9", "ghost:down", "2");
    // A key that is NOT a viewer's ratings hash must not be read as one.
    await r.call("HSET", "fablevideo:rating_counts_backup", "vid-1", "up");
  ...
  it("recounts across more viewers than one SCAN page returns", async () => {
    ...
    await r.call("DEL", "fablevideo:rating_counts");
