...
const { emailEnabled, sendAccessRequestEmail, pushEnabled, sendPushToEmails, sendPushToApproved, capabilityHolders } =
  vi.hoisted(() => ({
    emailEnabled: vi.fn(),
...
beforeEach(() => {
  ...
  capabilityHolders.mockResolvedValue(["root@example.com", "boss@example.com"]);
...
describe("who it reaches", () => {
  it("emails every holder of the people-management capability", async () => {
    await notifyNewAccessRequest(request);
    expect(capabilityHolders).toHaveBeenCalledWith("people.manage");
  ...
  it("sends nothing when nobody holds the capability", async () => {
    capabilityHolders.mockResolvedValue([]);
    await notifyNewAccessRequest(request);
...
describe("inert until configured", () => {
  it("does nothing at all with neither email nor push configured", async () => {
    ...
    expect(capabilityHolders).not.toHaveBeenCalled();
...
describe("best-effort", () => {
  ...
  it("does not throw when the recipient lookup fails", async () => {
    capabilityHolders.mockRejectedValue(new Error("redis is down"));
    const result = await notifyNewAccessRequest(request);
    expect(result).toEqual({ emailed: 0, pushed: 0, recipients: 0 });
    expect(sendAccessRequestEmail).not.toHaveBeenCalled();
  });
