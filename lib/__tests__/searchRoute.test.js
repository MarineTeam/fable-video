...
vi.mock("../videoList", () => ({
  fetchVideoLibrary: async (scope, options) => {
    ...
    scopeSeen = scope;
    capSeen = options?.cap;
    return { videos: libraryVideos, thumbnails: true, libraryTruncated };
  },
}));
...
describe("a library larger than one read", () => {
  it("says the oldest videos were not searched", async () => {
    libraryTruncated = true;
    expect((await search("sunday")).body.libraryTruncated).toBe(true);
  });

  it("does not say so for a library read whole", async () => {
    expect((await search("sunday")).body.libraryTruncated).toBe(false);
  });
});
