...
const pruneGroupFromSchedules = vi.fn(async () => 0);
...
// Group ids come from group NAMES, so a later group of the same name would
// inherit every per-group publish window a deleted one had.
describe("deleting a group", () => {
  it("clears its per-group publish windows", async () => {
    groups = [{ id: "youth", name: "Youth", restricted: false, videoIds: [], collectionIds: [] }];
    const res = await callRoute(route, { method: "DELETE", query: { name: "Youth" } });
    expect(res.statusCode).toBe(200);
    expect(pruneGroupFromSchedules).toHaveBeenCalledWith("youth");
  });

  it("still deletes the group when clearing the windows fails", async () => {
    groups = [{ id: "youth", name: "Youth", restricted: false, videoIds: [], collectionIds: [] }];
    pruneGroupFromSchedules.mockRejectedValueOnce(new Error("redis down"));
