// saveProgress (lib/store.js) against a REAL redis-server: the capped write
...
const { saveProgress, getProgress } = await import("../store");
const { MAX_PROGRESS_ENTRIES } = await import("../progress");

const KEY = "fablevideo:progress:jane@example.com";
const at = (n) => new Date(Date.UTC(2026, 0, 1) + n * 60_000).toISOString();

async function fill(n) {
  const args = [];
  for (let i = 0; i < n; i += 1) args.push(`v${i}`, JSON.stringify({ t: 1, d: 10, at: at(i) }));
...
describe.skipIf(shouldSkip)("saveProgress on a real redis-server", () => {
  ...
  it("writes an entry that reads back as it was saved", async () => {
    await saveProgress("jane@example.com", "vid-1", { t: 30, d: 600, at: at(1) });
    expect(await getProgress("jane@example.com")).toEqual({ "vid-1": { t: 30, d: 600, at: at(1) } });
  });

  it("updates a video already at the cap without evicting anything", async () => {
    await fill(MAX_PROGRESS_ENTRIES);
    await saveProgress("jane@example.com", "v500", { t: 99, d: 10, at: at(5000) });
    expect(Number(await r.call("HLEN", KEY))).toBe(MAX_PROGRESS_ENTRIES);
    expect(parse(await r.call("HGET", KEY, "v500")).t).toBe(99);
  ...
  it("makes room for a new video at the cap by dropping the least recently watched", async () => {
    await fill(MAX_PROGRESS_ENTRIES);
    await saveProgress("jane@example.com", "brand-new", { t: 5, d: 10, at: at(5000) });
    expect(Number(await r.call("HLEN", KEY))).toBe(MAX_PROGRESS_ENTRIES);
