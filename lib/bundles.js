...
async function tagSharesWithBundle(itemIds, bundleId) {
  await Promise.all(
    itemIds.map((itemId) => updateShare(itemId, { bundleId }).catch(() => {}))
  );
}
...
export async function liveBundleItems(bundle, id) {
  ...
  const records = await Promise.all(ids.map((itemId) => getShare(itemId)));
  const live = [];
  const liveIds = [];
  ids.forEach((itemId, i) => {
    if (isShareLive(records[i])) {
      live.push({ id: itemId, ...records[i] });
      liveIds.push(itemId);
    }
  });
