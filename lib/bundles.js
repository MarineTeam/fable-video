...
// Deletes a batch of bundles outright — the bundle record and its
// email -> bundle pointer — given the {id, email} pairs an admin cleanup
// pass has already identified as holding zero live items (see
// pages/api/admin/cleanup.js). Not used by any per-recipient read path:
// those always go through getBundle/getBundleForEmail, which already treat
// a missing key as "no bundle," so deleting one here is safe and final.
export async function deleteBundlesById(entries) {
  if (!entries.length) return 0;
  const keys = entries.flatMap(({ id, email }) =>
    email ? [bundleKey(id), emailPointerKey(email)] : [bundleKey(id)]
  );
