...
// Linear-time, deliberately NOT the single regex it replaced
// (/^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/). Because `.` is itself matched by
// [^\s@], the split between the two quantified runs around the literal dot is
// ambiguous, so a long no-dot input makes the engine retry every split —
// polynomial backtracking, which CodeQL flags as ReDoS. This matters because
// parseEmailList() below runs it over pasted bulk address lists.
//
// Same accept/reject set as the regex, including the >= 2 character TLD, so
// nothing that used to validate stops validating.
export function isValidEmail(email) {
  ...
  // The regex accepts the domain iff SOME dot has >= 1 char before it and >= 2
  // after — and both [^\s@] runs match dots themselves, so 'b@..bb.' and
  // 'a@b.b.c' were both accepted. The earliest dot at index >= 1 leaves the
  // longest tail, so testing that one alone decides it. Neither lastIndexOf
  // nor a plain indexOf reproduces this; both silently tighten the rule.
  // This is a ReDoS fix, not a validation change.
