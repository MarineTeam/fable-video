...
//
// Three independent lists, all env-var-based (edited directly in Vercel —
// no dependency on the app itself being reachable):
//
...
// - ADMIN_GEO_BYPASS_EMAILS — an identity-based bypass: a signed-in visitor
//   whose email is in this list always gets through, regardless of country
//   and regardless of either toggle above. Unlike ADMIN_GEO_WHITELIST this
//   doesn't require knowing which country you'll be in ahead of time, so
//   it's meant to be armed once, before traveling, as a standing safety
//   net — not something you edit in the moment, since env var changes only
//   take effect on redeploy.
//
// GEO_WHITELIST and ADMIN_GEO_WHITELIST are each gated by their own
// Redis-backed enforcement toggle (Settings tab, off by default);
// ADMIN_GEO_BYPASS_EMAILS has no toggle — presence on the list is enough.
//
// All three lists are read-only from the app's side — the Settings tab
// only displays them (see pages/api/admin/settings.js), it never edits
// them.
...
export function adminGeoBypassEmails() {
  return (process.env.ADMIN_GEO_BYPASS_EMAILS || "")
    .split(",")
...
// Pure decision function: given a resolved country code, a signed-in email
// (if any), the two enforcement toggles, and the three lists, decides
// whether the request is let through. Kept side-effect free and
// synchronous so it's testable without touching Redis or env vars.
export function resolveGeoAccess({
  ...
  email,
  adminGeoBypassEmails: bypassEmails = [],
}) {
  if (email && bypassEmails.includes(normalizeEmail(email))) return true;
