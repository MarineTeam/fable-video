...
// Geo-location whitelist — admin-managed like everything else in this file,
// no redeploy needed. Restriction only takes effect once enabled AND at
// least one country is listed (see lib/geo.js's geoRestrictionActive).
export async function getGeoSettings() {
  const raw = (await redis().hgetall(k("settings"))) || {};
  return { enabled: raw.geoEnabled === true || raw.geoEnabled === "true" };
}
...
export async function listAllowedCountries() {
  return ((await redis().smembers(k("geo", "countries"))) || []).sort();
}

export async function addAllowedCountry(code) {
  await redis().sadd(k("geo", "countries"), code);
}

export async function removeAllowedCountry(code) {
  await redis().srem(k("geo", "countries"), code);
}
