...
// lives under the VIEWER's key so removing a viewer takes their votes with
// them, and the counters hold no identity at all.
...
export async function setRating(email, videoId, vote) {
  await redis().hset(k("ratings", email), { [videoId]: vote });
}

export async function clearRating(email, videoId) {
  await redis().hdel(k("ratings", email), videoId);
}
...
// lib/ratings.js clamps a negative back to zero, and the drift is recorded as
// a known weak point rather than pretended away.
export async function applyRatingCounts(deltas) {
  for (const [field, delta] of Object.entries(deltas || {})) {
    if (!field || !delta) continue;
    try {
      await redis().hincrby(k("rating_counts"), field, delta);
    } catch (err) {
      console.error("Could not update a rating counter:", err);
    }
  }
}
