...
// All currently-live shares for one video — the data behind the per-video
// "Private list" admin panel (pages/api/admin/video-shares.js). Nothing
// stops the same {video, email} pair from having more than one live share
// (e.g. one made here, another via the single-share or bulk-share flows) —
// see groupSharesByEmail below for how the UI collapses that to one row.
export async function listLiveSharesForVideo(videoId) {
  const all = await listShares();
  return all.filter((s) => s.videoId === videoId && isShareLive(s));
}

// Pure — collapses a video's live shares to one row per recipient for the
// "Private list" UI, in case more than one live share exists for the same
// {video, email} pair. `ids` carries every underlying share id so removing
// a person from the list can revoke all of them at once, not just one.
