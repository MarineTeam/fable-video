...
function buildShare({ videoId, videoTitle, email, ttlHours, now, watermark, privateList }) {
  return {
    videoId,
    videoTitle,
    email,
    ...
    revoked: false,
    revokedAt: null,
    // True only for a share created through the per-video "Private list"
    // panel (pages/api/admin/video-shares.js) — never set by the ad hoc
    // Share/Bulk Share flows. listPrivateListSharesForVideo() filters on
    // this so that panel's listing, its "already on the list" dedup, and
    // its Remove action only ever see/touch shares it created itself. A
    // separate Share/Bulk Share link to the same video+email is a
    // completely independent, independently-revocable record that the
    // list is deliberately blind to.
    privateList: Boolean(privateList),
  };
}
...
// `privateList: true` tags every share created in this call as belonging to
// the per-video Private list panel (see buildShare above) — the caller is
// responsible for passing it only from that panel's own route.
export async function createShares(pairs, { hours, createdBy, watermark, privateList }) {
  const ttlHours = clampShareHours(hours);
  const now = Date.now();
  const created = pairs.map(({ videoId, videoTitle, email }) => ({
    id: crypto.randomBytes(16).toString("base64url"),
    share: {
      ...buildShare({ videoId, videoTitle, email, ttlHours, now, watermark, privateList }),
      createdBy,
    },
  }));
  if (created.length) {
...
// The live shares behind one video's "Private list" admin panel
// (pages/api/admin/video-shares.js) — deliberately only the ones tagged
// privateList: true, i.e. created through that panel itself. A share to the
// same video+email made via the ad hoc Share/Bulk Share flows is a separate,
// independently-revocable record this deliberately never sees: the panel's
// listing, its "already on the list" add-dedup, and its Remove action all
// stay scoped to what the panel itself created, never reaching into shares
// it didn't make. Nothing stops more than one live privateList share
// existing for the same {video, email} pair (e.g. removed then re-added
// while the old one's grace window hasn't lapsed) — see groupSharesByEmail
// below for how the UI collapses that to one row.
export async function listPrivateListSharesForVideo(videoId) {
  const all = await listShares();
  return all.filter((s) => s.videoId === videoId && s.privateList && isShareLive(s));
}

// Pure — collapses a video's private-list shares to one row per recipient
// for the "Private list" UI, in case more than one live share exists for
// the same {video, email} pair. `ids` carries every underlying share id so
// removing a person from the list can revoke all of them at once, not just
// one.
export function groupSharesByEmail(shares) {
  const byEmail = new Map();
  for (const share of shares) {
    const existing = byEmail.get(share.email);
    if (!existing) {
      byEmail.set(share.email, {
        email: share.email,
        ids: [share.id],
        createdAt: share.createdAt,
        expiresAt: share.expiresAt,
        emailedAt: share.emailedAt || null,
        viewCount: share.viewCount || 0,
      });
      continue;
    }
    existing.ids.push(share.id);
    if (share.createdAt < existing.createdAt) existing.createdAt = share.createdAt;
    if (share.expiresAt > existing.expiresAt) existing.expiresAt = share.expiresAt;
    if (share.emailedAt && (!existing.emailedAt || share.emailedAt > existing.emailedAt)) {
      existing.emailedAt = share.emailedAt;
    }
