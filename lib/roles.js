...
import { allowedVideoIds, groupId } from "./groups";
import { getViewerMeta } from "./store";
import { scopeForEmail, staffVideoScope } from "./staffScope";
import { capabilitiesUnderScope } from "./staffScopeRules";
...
export async function clearRolesForEmail(email) {
  ...
  await Promise.all([
    ...
    // Their group scope goes with their roles: a scope without roles limits
    // nothing, and would silently re-apply if roles were given back later.
    r.hdel(k("user:scope"), normalized).catch(() => {}),
  ]);
}
...
//
//   staffScope === null  -> staff across the whole portal (or not staff)
//   staffScope           -> the only group ids a scoped staff member's
//                           capabilities reach (lib/staffScopeRules.js)
//
...
export async function resolveAccess(email, { viewerMeta } = {}) {
  ...
  if (isEnvAdmin(normalized)) {
    return {
      ...
      videoScope: null,
      groupIds: [],
      // Owners are never scoped, whatever k("user:scope") holds.
      staffScope: null,
    };
  }
  ...
  // Group-scoped staff. A scope strips the portal-wide capabilities and
  // limits the library to what the scope's groups may watch. Read only for
  // someone who holds a capability; a Redis failure here denies, like every
  // other failure on this path — never "unscoped".
  let staffScope = null;
  let scopedVideos = null;
  if (hasAnyCapability(capabilities)) {
    try {
      staffScope = await scopeForEmail(normalized);
      if (staffScope) {
        capabilities = capabilitiesUnderScope(capabilities, staffScope);
        scopedVideos = await staffVideoScope(staffScope);
      }
    } catch (err) {
  ...
  // Unscoped staff need the whole library to do their job; scoped staff get
  // what their groups may watch; everyone else is scoped by their own groups.
  let videoScope = null;
  if (staff && staffScope) {
    videoScope = scopedVideos;
  } else if (!staff) {
    staffScope = null;
    try {
  ...
  // The groups this viewer belongs to, by id — what per-group publish windows
  // are keyed on (lib/schedule.js isLiveFor). Derived from the tags already
  // read above, so it costs nothing. Every tag is included, grouped or not:
  // a window keyed on a tag with no group record simply never exists.
  const groupIds = [...new Set((meta?.tags || []).map(groupId).filter(Boolean))];
  ...
  return {
    ...
    videoScope,
    groupIds,
    staffScope: staff ? staffScope : null,
  };
}
