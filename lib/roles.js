...
// Everyone who currently holds a capability: the ADMIN_EMAILS seed (always
// admin, so it holds everything) plus every stored role that grants it.
//
// Derived from the role/capability table rather than hardcoding "admins", so
// if a capability is ever moved between roles the notification recipients
// follow it automatically instead of silently going to the wrong people.
export async function capabilityHolders(capability) {
