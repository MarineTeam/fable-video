...
// in its default language (`spokenText`), or in a translation (`translated`,
// true when the store found the query in one; see matchingTranslatedIds).
export function videoMatches(video, query, spokenText, translated = false) {
  if (videoMatchesQuery(video, query)) return true;
  if (spokenMatches(spokenText, query)) return true;
  return translated === true && normalizeSpoken(query).length >= MIN_TRANSCRIPT_QUERY;
}
...
//
// `translatedIds` is the set of video ids whose TRANSLATIONS contain the query
// (from the store). It can only mark videos already in `videos`; an id for
...
export function searchLibrary({
  ...
  const translated = translatedIds instanceof Set ? translatedIds : new Set();

  const hits = list.filter((video) =>
    videoMatches(video, q, texts[video?.id], translated.has(String(video?.id)))
  );
