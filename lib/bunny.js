...
// A whole-library read stops after this many pages (100 videos each, so
// 1,000 videos) and says so: `truncated` is true past it, for the Videos tab,
// Analytics and search to report rather than present part of the library as
// all of it. It was 5 pages (500 videos) and said nothing.
export const MAX_LIBRARY_PAGES = 10;

// The first page tells us the total count, so any remaining pages are
// fetched in parallel instead of one page at a time. Pages are counted in the
// size bunny actually served — itemsPerPage is a request, not a promise.
async function fetchAllVideosUncached() {
  const first = await listVideos({ page: 1 });
  const items = first?.items || [];
  const total = first?.totalItems ?? items.length;
  if (!items.length || items.length >= total) {
    return { videos: items, truncated: false, total: Math.max(total, items.length) };
  }

  const needed = Math.ceil(total / items.length);
  const totalPages = Math.min(MAX_LIBRARY_PAGES, needed);
  const rest = await Promise.all(
    Array.from({ length: totalPages - 1 }, (_, i) => listVideos({ page: i + 2 }))
  );
  const videos = [...items];
  // An upload landing between two page reads can push a video across a page
  // boundary, so the same guid can arrive twice.
  const seen = new Set(items.map((video) => video?.guid));
  ...
  return { videos, truncated: needed > MAX_LIBRARY_PAGES, total };
}
...
//
// Returns { videos, truncated, total }. listAllVideos() below is the same
// read, videos only, for the callers that have nothing to report.
export function listAllVideosWithStatus() {
  const now = Date.now();
  if (videoListCache && now - videoListCache.at < VIDEO_LIST_CACHE_TTL_MS) {
    return videoListCache.promise;
  }
  const promise = fetchAllVideosUncached();
...
export async function listAllVideos() {
  return (await listAllVideosWithStatus()).videos;
}
