...
import { clearTranscript } from "./captionsStore";
import { clearComments } from "./commentsStore";

const chaptersKey = () => k("chapters");
const notesKey = () => k("notes");
// Owned by lib/captionsStore.js — named here only so pruneVideoMeta can clear
// them on delete. Importing that module instead would make this file and it
// mutually dependent for two key names.
const transcriptsKey = () => k("transcripts");
const transcriptTextKey = () => k("transcript_text");
...
// The single per-video cleanup hook, called from both delete paths in
// pages/api/admin/videos.js. Every per-video decoration belongs here rather
// than at the call sites: a decoration added later is one line in one place,
// and forgetting it leaves rows for a video that no longer exists with nothing
...
export async function pruneVideoMeta(id) {
  ...
  await Promise.all([
    ...
    r.hdel(transcriptsKey(), key).catch(() => {}),
    r.hdel(transcriptTextKey(), key).catch(() => {}),
    // Every language, not just the default: clearTranscript reads the
    // language index to find the translation rows (cues and search text),
    // which the two hdels above cannot see.
    clearTranscript(key).catch(() => {}),
    // A recycled id must not open with a previous video's conversation.
    clearComments(key).catch(() => {}),
  ]);
}
