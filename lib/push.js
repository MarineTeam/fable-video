...
import { effectiveCapabilities, loadRoleAssignments, loadRoles } from "./roles";
  ...
  await Promise.all(
    targets.map(async ({ endpoint, sub }) => {
      ...
      try {
        ...
        recordExternal("webpush /send", Number(process.hrtime.bigint() - start) / 1e6);
      }
    })
  );
...
// Sends to every subscription whose owner is currently an approved viewer,
// an ADMIN_EMAILS admin, or holds a staff role (manager/admin) in Redis —
// staff are implicitly approved without being on the viewer list, so they
// must be resolved here too. Someone removed from the viewer list, or
// stripped of every role, stops receiving on the next send even if their old
// subscription record is still around.
export async function sendPushToApproved(payload) {
  if (!pushEnabled()) return { sent: 0, pruned: 0, configured: false };
  const [subs, viewers, rolesById, assignments] = await Promise.all([
    listPushSubscriptions(),
    listViewers().catch(() => []),
    loadRoles().catch(() => ({})),
    loadRoleAssignments().catch(() => ({})),
  ]);
  const allowed = new Set([
    ...viewers.map((v) => v.email),
    ...adminEmails(),
    // Staff is now "holds at least one capability" rather than a role name:
    // someone an owner trusted with any admin capability can watch, so they
    // are approved for the purposes of a broadcast too.
    ...Object.entries(assignments)
      .filter(([, roleIds]) => effectiveCapabilities({ roleIds, rolesById }).length > 0)
      .map(([email]) => email),
  ]);
...
// Targeted send: only to subscriptions owned by one of the given addresses.
//
// Deliberately NOT a variant of sendPushToApproved. That one broadcasts to
// everyone approved, which is right for "a new video is up" and wrong for
// anything addressed to a subset — telling every viewer that a named stranger
// asked for access would leak both the request and the requester. Callers
// pass the exact recipient list they mean.
export async function sendPushToEmails(emails, payload) {
  if (!pushEnabled()) return { sent: 0, pruned: 0, configured: false };
  const allowed = new Set(
    (Array.isArray(emails) ? emails : []).map((e) => normalizeEmail(e)).filter(Boolean)
  );
  if (!allowed.size) return { sent: 0, pruned: 0, configured: true };
  const subs = await listPushSubscriptions();
  const targets = subs.filter((s) => s.email && allowed.has(normalizeEmail(s.email)));
  return deliver(targets, payload);
}
