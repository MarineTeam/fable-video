...
import { isStaffRole, listStoredRoles } from "./roles";
...
// demoted to plain viewer, stops receiving on the next send even if their
// old subscription record is still around.
export async function sendPushToApproved(payload) {
  if (!pushEnabled()) return { sent: 0, pruned: 0, configured: false };
  const [subs, viewers, roles] = await Promise.all([
    listPushSubscriptions(),
    listViewers().catch(() => []),
    listStoredRoles().catch(() => ({})),
  ]);
  const allowed = new Set([
    ...viewers.map((v) => v.email),
    ...adminEmails(),
    ...Object.entries(roles)
      .filter(([, role]) => isStaffRole(role))
      .map(([email]) => email),
  ]);
