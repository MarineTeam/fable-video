...
//
// PURE MODULE — no Redis, no fetch. The homepage and the search route both
// match through lib/notes.js, which imports this, so the two halves of a
// search agree about what a passage matches; the watch page imports it to
// show a video's passages as links. Same bundle rule as lib/notes.js.
//
...
// says where a video's references come from — each repo passes the same
// source its passage search matches against, so a book listed here is a
// book that clicking will find.
//
// The caller decides WHICH videos: always the viewer's already-scoped
// library. A count is itself information — "Philippians (3)" says three
// videos exist — so an index built over videos the viewer cannot see would
// leak exactly what the scope exists to hide.
