...
pages/
  ...
  _document.js            No-flash palette script (applies cached theme pre-paint);
                           resolves the live site name server-side for the
...
lib/
  ...
  __tests__/              Vitest tests — pure logic plus resolveAccess, the
                          manifest route, and the first API route coverage
                          (helpers/route.js harness)
styles/globals.css        Design system (dark glassmorphism, gradient accents, Inter)
public/
  (manifest.webmanifest is generated by pages/api/manifest.js — see below;
   no static file lives here anymore)
...
- **An installed app doesn't show the new name right after a rename** — not a
  bug. `/manifest.webmanifest` and the `apple-mobile-web-app-title` meta tag
  are both generated live from the admin-set name (`pages/api/manifest.js`,
  `pages/_document.js`), so a fresh install always gets the current name. An
  *already-installed* app only picks it up on whatever schedule the OS/browser
  re-checks an installed PWA's manifest — that varies by platform and isn't
  something the app controls, and can take a few app opens or several days.
  The app icons themselves are static image files and always need a file
  edit + redeploy to change, regardless of the name.
