...
- **Fixing the symptom without reading the log line.** Every catch block in `pages/api/**`
  logs before it returns a 5xx (rule added in `3d5a050` specifically because the previous
...
Written 2026-07-13 against commit `1be60d7` (v1.6.0, first release 2026-07-07). Every
...
|---|---|
| Every admin catch logs before its 5xx (rule from `3d5a050`) | `grep -L "console.error" pages/api/admin/*.js` (expect no output — a hit means a route is missing the log) |
