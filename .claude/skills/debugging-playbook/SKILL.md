...
| The "🔔 Notify me" button never appears for a logged-in, approved viewer | No server log — the button self-hides client-side | `components/PushToggle.js` | One of: (a) `NEXT_PUBLIC_VAPID_PUBLIC_KEY` is unset (so `pushEnabled()`/the client key check is false — the whole feature is inert); (b) the browser lacks `serviceWorker`/`PushManager`/`Notification` (older/embedded browsers); (c) notifications are already `denied` in the browser (then a "blocked in browser settings" chip shows instead); (d) **on iOS**, the site isn't installed to the Home Screen yet — iOS only exposes Web Push to an installed PWA (16.4+) | Confirm `NEXT_PUBLIC_VAPID_PUBLIC_KEY` is set **and the deploy that set it was redeployed** (it's inlined at build time, so a value added after the last build won't appear until a rebuild). On iOS, install the app first. Check `Notification.permission` in the browser console — if `"denied"`, the user must re-allow in browser/site settings | README "Optional — push notifications" and "Common issues"; `components/PushToggle.js`; `lib/push.js` `pushEnabled()` |
| Push notifications aren't arriving, or an admin broadcast reports `sent: 0` | Admin broadcast returns `{ ok: true, sent: 0 }`; auto-announce logs `"New-video announce failed:"` only on an actual throw (best-effort) | `pages/api/admin/notify.js`, `lib/push.js` (`sendPushToApproved`, `maybeAnnounceReadyVideos`) | Ordered by likelihood: (1) `VAPID_PRIVATE_KEY`/`NEXT_PUBLIC_VAPID_PUBLIC_KEY` not set on the deployment doing the sending (`503` from `/api/admin/notify`, or a silent no-op from the announcer); (2) **no currently-approved viewer has an active subscription** — `sendPushToApproved` filters subs against the live viewer list + `ADMIN_EMAILS`, so a de-approved viewer's stored sub is skipped (`sent` counts only delivered); (3) subscriptions went stale — dead endpoints return 404/410 and are pruned (`pruned > 0` in the result); (4) the VAPID **public** key changed after users subscribed, invalidating their `applicationServerKey` | Check `/api/admin/notify`'s JSON (`sent`/`pruned`) and Vercel logs. Verify both VAPID vars are set on the live deployment (`check-env.mjs`). Confirm at least one **currently-approved** viewer actually subscribed (they clicked "Notify me" and it turned to "Notifications on"). If you rotated the VAPID keypair, every prior subscription is invalid — users must re-subscribe | README "Optional — push notifications"; `lib/push.js`; `domain-reference` §8; `architecture-contract` invariant (l) |
| New-video notifications never fire even though push is configured and viewers subscribed | No error — `maybeAnnounceReadyVideos` seeded silently | `pages/api/admin/videos.js` → `lib/push.js` `maybeAnnounceReadyVideos` | **By design on first run:** the first time the announcer runs after push is enabled, it `SADD`s every already-ready video into `fablevideo:push:notified` and sets `fablevideo:push:seeded` **without sending**, so the whole existing library isn't blasted. Only videos that become ready *after* that seed run trigger a notification. If `fablevideo:push:seeded` exists but `notified` is missing a genuinely-new video, the announce path (which only runs when the admin video-list route is hit) may simply not have executed yet | Not a bug for existing videos — only newly-ready videos announce. To confirm the machinery, upload a new video and load the admin Videos tab (which triggers `maybeAnnounceReadyVideos`) once it encodes to ready. Inspect `fablevideo:push:*` keys via `diagnostics-and-tooling`'s `check-redis.mjs` | `lib/push.js` (`maybeAnnounceReadyVideos`, seed sentinel); `domain-reference` §8 |
...
**Updated 2026-07-15 (v1.8.0 Web Push):** added three symptom rows — "Notify me" button
missing, notifications not arriving / broadcast `sent: 0`, and new-video announce silent on
first run — verified against `components/PushToggle.js`, `lib/push.js`,
`pages/api/admin/notify.js`, and `pages/api/admin/videos.js`.
...
| Push is inert without both VAPID vars; `/api/admin/notify` is 10/hour | `grep -n "pushEnabled\|allowRequest" lib/push.js pages/api/admin/notify.js` |
| Sends filter against the live viewer list (de-approved viewers skipped) | `sed -n '99,113p' lib/push.js` |
