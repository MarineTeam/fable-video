...
| **Shares are TTL-native Redis records plus an index set with opportunistic pruning — and, since the Extend feature, "expired" is an app-level check, not the record's physical deletion.** `createShare` sets the record's physical Redis TTL to `ttlHours * 3600 + GRACE_SECONDS` (`lib/shares.js`, `GRACE_SECONDS = 30 days`), deliberately **longer** than the share's nominal `expiresAt`. Every recipient-facing read (the watch page, the bundle page, `share-track`) must call `isShareLive(share)` — `expiresAt` in the future — instead of treating "the record exists" as "the link works"; `listShares()` and the admin Shares tab intentionally keep showing grace-window-expired records so they can be Extended. `updateShare` still reads the record's *remaining physical* TTL via `r.ttl(key)` and re-sets with that same remaining value, never resetting the clock on a view/email stamp. `extendShare` is the one function allowed to move `expiresAt` itself, always "from now." `listShares` (`lib/shares.js`) reads the index, `mget`s all records, and opportunistically `srem`s any id whose record has fully left the grace window. | Before Extend existed, a share's physical Redis TTL *was* its expiry — the record simply vanished the instant it lapsed, which is why `updateShare` was careful to preserve remaining TTL rather than reset it (that invariant still holds, unchanged). Extend added a real requirement — "push an already-expired-but-not-revoked link back to live" — that's impossible if Redis has already hard-deleted the record. The fix (grace window + app-level `isShareLive`) is deliberate and narrow: revoke still does an immediate, un-graced `DEL`, so "no record at all" continues to mean exactly "revoked, or genuinely never existed, or aged out past 30 days" — Extend naturally refuses all three by finding nothing to extend, with no separate "revoked" flag needed. Any new recipient-facing read path must call `isShareLive`, not just check truthiness, or it will silently serve a link that's nominally expired but still inside its grace window. |
