...
| `fablevideo:ratings:<email>` | hash, field = video GUID, value = `"up"` or `"down"` | `setRating()`/`clearRating()` (`lib/store.js`, via `pages/api/rating.js`) | `getRatings()` → `ratingOf()` (`lib/ratings.js`) — the watch page's 👍/👎 buttons (server-rendered) | none. Per-viewer, keyed by email like `progress:` and `mylist:`, **so removing a viewer takes their votes with them** — the reason the vote is not stored under the video |
| `fablevideo:rating_counts` | hash, field = `<video GUID>:up` / `<video GUID>:down`, value = integer | `applyRatingCounts()` (`lib/store.js`) — `HINCRBY` of a delta from `voteDelta()`, **after** the vote is written and best-effort | `getRatingCounts()` → `countsByVideo()` (`lib/ratings.js`) — the admin Videos tab badge, **staff only** | `clearVideoRatingCounts()` on video delete (single and bulk), so a recycled GUID cannot inherit a score. Holds **no identities at all**. Can drift by one against the votes and is never recomputed (see FEATURES.md "Known gaps"); a negative reads as 0 |
