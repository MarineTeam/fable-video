...
| `fablevideo:transcribe_pending` | hash, field = video GUID, value = epoch ms when transcription was queued | `markTranscribePending()` (`pages/api/admin/transcribe.js`, on a successful queue) | `getTranscribePending()` → `planCollection()` (`lib/transcribeQueue.js`), read by the admin video list to collect finished jobs | cleared on ingest (manual or automatic) and when a marker passes `PENDING_MAX_AGE_MS` (24h). A MARKER, not a job — nothing runs it; the admin list does the work on a request that was happening anyway |
