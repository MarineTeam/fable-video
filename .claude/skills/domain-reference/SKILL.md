...
| `fablevideo:roles` | hash, field = **roleId**, value = `{id, name, capabilities[], createdAt, updatedAt}` | `saveRole()` (`pages/api/admin/roles.js`) | `loadRoles()` → `resolveCapabilities()`/`resolveAccess()` (`lib/roles.js`) — the authorization gate | none. **Legacy shape warning:** before custom roles this same hash held `email -> "manager"\|"admin"`. Leftover rows are inert (`parseRole` rejects a bare string; `isValidRoleId` rejects any field with `@` or `.`) and are converted by `lib/roleMigration.js` |
| `fablevideo:user:roles` | hash, field = normalized email, value = `[roleId, ...]` (max 10) | `setRolesForEmail()` (`pages/api/admin/roles.js` PATCH) | `rolesForEmail()`, `loadRoleAssignments()` — which roles a person holds | none. An empty list deletes the field |
| `fablevideo:user:scope` | hash, field = normalized email, value = JSON array of group ids (max 20) | `setScopeForEmail()` (`pages/api/admin/roles.js` PATCH `scope`); `pruneGroupFromScopes()` on group delete | `scopeForEmail()` → `resolveAccess().staffScope` (`lib/staffScope.js`) — the group limit on a staff member | none. **Absent = the whole portal; `[]` = no groups.** A deleted group is removed from the list, and an emptied list is stored as `[]`, never deleted — deleting would widen the person to everything. Cleared with their roles |
...
| `fablevideo:transcripts` | hash, field = video GUID, value = JSON array of `{start, end, text}` (seconds) | `setTranscript()` (`pages/api/admin/transcribe.js`, `ingest: true`; VTT fetched server-side from the pull zone and parsed by `lib/captions.js`) | `getTranscript()` (`lib/captionsStore.js`) — served by `pages/api/transcript/[id].js` to the watch page's transcript panel | none. `pruneVideoMeta()` clears it when the video is deleted |
| `fablevideo:transcript_text` | hash, field = video GUID, value = the same words as one plain string | written together with `fablevideo:transcripts` by `setTranscript()` | `getTranscriptTextMap()` (`lib/captionsStore.js`) — library search by what was **said**. Separate from the cue hash on purpose: search needs one `hgetall` and none of the timings, and cues are bulky (~1,500 per 90-minute service) | none. Cleared with the cue hash |
| `fablevideo:transcripts_alt` | hash, field = `<video GUID>:<lang>`, value = JSON cue array | `setTranscriptLanguage()` (`lib/captionsStore.js`), written for every non-default track the collector finds | `getTranscript(id, lang)` when the language is not the video's default | cleared with the video's transcript, using the language index below to know the field names. Deliberately a SEPARATE hash: `getTranscribedIds()` reads `transcripts` field names with `hkeys`, so a `guid:lang` field there would be reported as a transcribed video that does not exist |
| `fablevideo:transcript_text_alt` | hash, field = `<video GUID>:<lang>`, value = that translation's words, ALREADY normalised by `normalizeSpoken` | `setTranscriptLanguage()` (`lib/captionsStore.js`), beside the cue row | `matchingTranslatedIds()` only — a Lua script walks the hash INSIDE Redis and returns just the matching video ids, so search in every language never loads the translations into the app. Stored pre-normalised because Lua's `lower()` is not Unicode-aware: JavaScript normalises both sides, and the script does a plain substring find | cleared with the video's transcript (`clearTranscript`), and on video delete via `pruneVideoMeta` → `clearTranscript` |
| `fablevideo:comments:<videoId>` | hash, field = comment id (`c` + base-36 time + random), value = JSON `{id, email, name, text, at}` | `addComment()` (`lib/commentsStore.js`) via `POST /api/comments` — a Lua script that refuses once the hash holds `MAX_COMMENTS_PER_VIDEO` (500), so a cap cannot be raced | `listComments()` / `getComment()` via `GET` and `DELETE /api/comments`; the email is compared for "is this mine" and shown only to `viewers.read` holders | per-comment HDEL on delete; the whole key on video delete (`pruneVideoMeta` → `clearComments`). NOT removed with a viewer (see the FEATURES gap) |
| `fablevideo:transcript_langs` | hash, field = video GUID, value = `{default, all[]}` | `setTranscriptLanguages()` after a collection | `getTranscriptLanguages()` — the panel's picker, and how a read knows which language the default track is | cleared with the transcript. Read BEFORE deletion, since it is the only record of which `transcripts_alt` fields exist |
| `fablevideo:transcribe_pending` | hash, field = video GUID, value = epoch ms when transcription was queued | `markTranscribePending()` (`pages/api/admin/transcribe.js`, on a successful queue) | `getTranscribePending()` → `planCollection()` (`lib/transcribeQueue.js`), read by the admin video list and the scheduled job (`/api/cron/transcripts`) to collect finished jobs | cleared on ingest (manual or automatic) and when a marker passes `PENDING_MAX_AGE_MS` (3 days — long enough for two daily scheduled runs). A MARKER, not a job — the admin list and the scheduled job do the work |
| `fablevideo:transcribe_collecting` | string, a random `lock-…` token | `acquireCollectLock()` (`lib/captionsStore.js`), SET NX EX 300 at the start of a collection run | the release script, which deletes it only if it still holds the caller's token | EXPIRES after 5 minutes. One collection run at a time — an admin page load and the scheduled job (or a doubly-delivered run) skip rather than collect the same video twice |
| `fablevideo:mylist:<email>` | hash, field = video GUID, value = epoch ms when saved | `saveToMyList()`/`removeFromMyList()` (`lib/store.js`, via `pages/api/mylist.js`) | `getMyList()` — the watch page's save toggle (server-rendered) and the homepage's "My list" row | none. Capped at `MAX_ITEMS` (200) by `lib/mylist.js` on read, and refused at the cap on write. Per-viewer, keyed by email exactly like `fablevideo:progress:<email>` |
| `fablevideo:ratings:<email>` | hash, field = video GUID, value = `"up"` or `"down"` | `recordRating()` (`lib/store.js`, via `pages/api/rating.js`) — `VOTE_SCRIPT` (`lib/ratingScripts.js`), which writes the vote AND moves the counters in one Redis script | `getRatings()` → `ratingOf()` (`lib/ratings.js`) — the watch page's 👍/👎 buttons (server-rendered); `recountRatings()` reads every one | none today. Per-viewer, keyed by email like `progress:` and `mylist:`, so deleting a person's votes is one key — **but `removeViewer` does not do it yet** (FEATURES.md "Known gaps") |
| `fablevideo:rating_counts` | hash, field = `<video GUID>:up` / `<video GUID>:down`, value = integer | `recordRating()` — in the same script as the vote, so the two cannot disagree; `recountRatings()` (`RECOUNT_SCRIPT`, from **Recount ratings** on the admin Videos tab) rebuilds and replaces it from the votes | `getRatingCounts()` → `countsByVideo()` (`lib/ratings.js`) — the admin Videos tab badge, **staff only** | `clearVideoRatingCounts()` on video delete (single and bulk), so a recycled GUID cannot inherit a score. Holds **no identities at all**. A negative (only possible from before the vote script) reads as 0 until a recount |
