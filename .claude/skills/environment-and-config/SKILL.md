...
| `BUNNY_CDN_HOSTNAME` | Optional | `lib/bunny.js:16` `cdnHostname()`, gates `thumbnailsEnabled()` (line 173-175); also `lib/bunnyMedia.js` `mediaEnabled()` | Unset → `thumbnailsEnabled()` is `false` → homepage falls back to a plain title list, no thumbnails; AND the podcast feed serves no episodes (a valid but empty feed, not an error) | No | Pull-zone host, e.g. `vz-xxxx-xxx.b-cdn.net`. The on/off switch for thumbnails, and now also for podcast enclosures. |
| `BUNNY_MP4_HEIGHT` | Optional | `lib/bunnyMedia.js` `mp4Height()` | Unset → `720`. Selects which `play_{height}p.mp4` fallback the podcast feed links to | No | Must be a height the bunny.net library actually generates. Requires **MP4 Fallback** enabled on the library, and bunny.net only generates MP4s for videos uploaded *after* that was turned on — older videos 404. |
...
| `SITE_NAME` | Optional | `lib/siteName.js` `envSiteName()`, via `resolveSiteName()` | Unset → falls back to `NEXT_PUBLIC_SITE_NAME`, then `"Marine Video Portal"` | No | **Only the starting value.** The admin-set name in Redis (`k("settings")` field `siteName`, Settings → Site name) wins over this. Reaches the header, every page title, and emails |
| `NEXT_PUBLIC_SITE_NAME` | Optional | `lib/siteName.js` `envSiteName()` | Unset → falls back to the built-in default | No — **browser-exposed by design** | Alternative spelling kept for existing deployments; `SITE_NAME` wins if both are set, and the admin-set name wins over either |
...
| `CRON_SECRET` | Optional (scheduled jobs) | `lib/cronAuth.js` `cronSecret()`, read by `pages/api/cron/transcripts.js` | Unset, or shorter than 16 characters (logged) → every `/api/cron/*` route answers 404; transcripts are collected only when an admin opens the Videos tab | **Yes** — anyone holding it can trigger the job | At least 16 random characters, e.g. `openssl rand -hex 24`. Vercel sends it as `Authorization: Bearer …` on every cron call; the schedule is in `vercel.json` (daily on Hobby, which refuses anything more frequent) |
...
| `BUNNY_CDN_HOSTNAME` | Thumbnails — `thumbnailsEnabled()` is `false` (`lib/bunny.js:173-175`) — and podcast episodes (`mediaEnabled()` is `false`, so feeds come back empty) | Playback, everything else — homepage just shows a title list instead of a thumbnail grid, and the feed is valid but has no items |
| `BUNNY_MP4_HEIGHT` | Nothing — defaults to 720 | Everything |
| `CRON_SECRET` | The scheduled transcript collector (`/api/cron/transcripts` answers 404) | Everything, including transcript collection when an admin opens the Videos tab and the *Fetch captions* button |
...
| `SITE_NAME` / `NEXT_PUBLIC_SITE_NAME` | Nothing — cosmetic, and only the *starting* value: an admin sets the real name in Settings → Site name. Unset falls back to `"Marine Video Portal"` | Everything |
