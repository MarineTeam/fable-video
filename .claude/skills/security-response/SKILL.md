...
six CodeQL alerts in its history: two real (both fixed), four false positives (open, with a
...
| #2 | Use of password hash with insufficient computational effort (High) | `lib/bunny.js`, inside `signEmbedUrl` (function starts line 165; `crypto.createHash("sha256")` at line 168, the `.update(...)` line CodeQL points at is line 169) | **FALSE POSITIVE** — open, dismiss with justification below | verify: `grep -n "createHash\|^export function signEmbedUrl" lib/bunny.js` |
| #3 | Use of password hash with insufficient computational effort (High) | `lib/bunny.js`, inside `signTusUpload` (function starts line 176; `crypto.createHash("sha256")` at line 179, `.update(...)` at line 180) | **FALSE POSITIVE** — open, dismiss with justification below | verify: `grep -n "createHash\|^export function signTusUpload" lib/bunny.js` |
| #4 | Use of password hash with insufficient computational effort (High) | `lib/bunny.js`, inside `thumbnailUrl` (function starts line 197; `crypto.createHash("sha256")` at line 206, `.update(...)` at line 207) | **FALSE POSITIVE** — open, dismiss with justification below | verify: `grep -n "createHash\|^export function thumbnailUrl" lib/bunny.js` |
...
| #6 | Use of password hash with insufficient computational effort (High) | `lib/bunnyMedia.js`, inside `signCdnPath` (function starts line 74; `crypto.createHash("sha256")` at line 84, `.update(...)` at line 85 — unchanged since the alert was raised) | **FALSE POSITIVE** — open, dismiss with the same justification (it is the identical bunny.net pull-zone URL-token formula as #4, in the new module the podcast feed uses) | verify: `grep -n "createHash\|^export function signCdnPath" lib/bunnyMedia.js` |
| #7 | Improper code sanitization (Medium) | `pages/_document.js`, the inline theme-boot `<script>` — was `${JSON.stringify(THEME_CACHE_KEY)}` | **REAL — FIXED.** `JSON.stringify` serializes data; it does not make a value safe to paste into code. Its output leaves `<` untouched, so a value containing `</script>` closes the tag early, and it passes U+2028/U+2029 through, which older parsers read as line terminators inside a string literal. Introduced by the pre-paint palette fix (PR #34) when it swapped a hardcoded key for the imported constant. Fixed with a local `jsLiteral()` that escapes those three characters. **Nothing was exploitable** — `THEME_CACHE_KEY` is the constant `"fablevideo:theme"` — which is precisely why the call site was fixed rather than the alert dismissed: what made it safe was the value, and the value is the part that can change | verify: `grep -n "JSON.stringify" pages/_document.js` (3 hits — two comment lines, one the call inside `jsLiteral`; none inside the boot-script template); emitted script is byte-identical, check with `npm run build && grep -o 'localStorage.getItem("fablevideo:theme")' .next/server/pages/500.html` |

**Line numbers re-checked 2026-09-18 and #2–#4 HAVE drifted**, by 18 lines each: they are
now 169, 180 and 207, not the 151/162/189 the alerts were originally raised at. The
formulas themselves are untouched — `lib/bunny.js` simply grew above them. #6's line 85
is unchanged. This is the drift this block warns about, so treat the greps as
authoritative over any number written here. Numbering here follows
...
- **Reading an untrusted request parameter without checking its TYPE** — the alert-#5
  ...
  that may be an array, since Next.js hands back `string[]` for a repeated query key and
  `String(["a","b"])` is `"a,b"` — a value nobody sent. Use `lib/params.js`
  (`oneString`/`oneTrimmed`/`oneNumber`/`isExplicitlyTrue`), which rejects wrong types
  rather than coercing them. Check: `grep -rnE "String\(req\.|Number\(req\." pages/`
  — every hit should be justified or converted.
...
| Alerts #2–#4 line numbers (169/180/207 as of 2026-09-18; drifted up 18 from the 151/162/189 the alerts were raised at) | `grep -n "createHash" lib/bunny.js` |
| Alert #6 still the same formula in the same function | `grep -n "createHash\|^export function signCdnPath" lib/bunnyMedia.js` |
| Alert #7 fix still in place | `grep -n "JSON.stringify" pages/_document.js` (expect exactly 3 hits: two in the explanatory comment, one the call inside `jsLiteral`. A hit inside the `` `try{var h=...` `` template literal means the fix was reverted) |
